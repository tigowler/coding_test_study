...
# 4*4 정사각형에 존재하는 각 물고기의 번호(없으면 -1)와 방향 값을 담는 테이블
...
def move_all_fishes(array, now_x, now_y):
    # 1번부터 16번까지 물고기를 차례대로 (낮은 번호부터) 확인
    for i in range(1, 17):
        # 해당 물고기 위치 찾기
        ...
        if position != None:
            ...
            for j in range(8):
                nx = x + dx[direction]
                ny = y + dy[direction]
                # 해당 방향으로 이동 가능하면 이동시키기
                if 0<=nx and nx<4 and 0<=ny and ny<4:
                    if not (nx==now_x and ny == now_y): # 상어의 위치가 아니라면,
...
def dfs(array, now_x, now_y, total):
    ...
    # 이제 다시 상어가 이동해야하므로, 이동 가능한 위치 찾기
