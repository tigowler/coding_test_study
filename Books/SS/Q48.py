...
array = [list(map(int, input().split())) for _ in range(n)] # 모든 상어의 위치와 방향 정보를 포함하는 2차원 리스트
directions = list(map(int, input().split())) # 모든 상어의 방향 정보
smell = [[[0, 0]] * n for _ in range(n)] # 각 위치마다 [특정 냄새의 상어 번호, 특정 냄새의 남은 시간]을 저장하는 2차원 리스트
priorities = [[] for _ in range(m)] # 각 상어의 회전 방향 우선순위 정보
for i in range(m):
    for j in range(4):
        ...
        for y in range(n):
            ...
            if array[x][y] != 0:
                ...
                for index in range(4):
                    ...
                    ny = y + dx[priorities[array[x][y] - 1][direction - 1][index] - 1]
                    if 0 <= nx and nx < n and 0 <= ny and ny < n:
                        if smell[nx][ny][1] == 0: # 냄새가 존재하지 않는 곳이면,
                            # 해당 상어의 방향 이동시키기
                            directions[array[x][y] - 1] = priorities[array[x][y] - 1][direction - 1][index] # TODO: ?
